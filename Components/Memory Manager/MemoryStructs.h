...
namespace Memory::Structs
{
	...
	inline Field StructPointer( const uintptr_t Offset, const std::string& Label, const Info* Structure )
	{
		...
		if ( Structure ) F.EmbeddedInfo = std::make_unique<Info>( *Structure );
	...
	inline Field EmbeddedStruct( const uintptr_t Offset, const std::string& Label, const Info* Structure )
	{
		...
		if ( Structure )
		{
			...
			F.EmbeddedInfo = std::make_unique<Info>( *Structure );
		}
	...
	inline Info LIST_ENTRY = Info{
		_( "LIST_ENTRY" ),
		{
			StructPointer( 0x0, _( "Flink" ), &LIST_ENTRY ),
			StructPointer( 0x8, _( "Blink" ), &LIST_ENTRY )
		}
	};
